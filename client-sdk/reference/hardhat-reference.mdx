...
| Method | Description |
| --- | --- |
| `hre.cofhe.createClientWithBatteries(signer?)` | Creates a fully configured `CofheClient` with a self-ACP. Uses the first signer if none provided. |
...
export default {
  solidity: '0.8.28',
  cofhe: {
    gasWarning: true,           // Warn on high mock gas usage (default: false)
    mocksDeployVerbosity: 'v',  // Mock deploy log verbosity: '' | 'v' | 'vv' (default: 'v')
  },
};
```
...
| Task | Description |
| --- | --- |
| `task:cofhe-mocks:deploy` | Deploy mock contracts. Takes no parameters. |
