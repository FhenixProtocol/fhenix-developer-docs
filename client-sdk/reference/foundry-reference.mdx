...
| `decryptForView(bytes32 ctHash, Permission permit)` | `uint256` | Offchain seal/unseal. **Reverts on deny**, to assert deny use `mockThresholdNetwork.querySealOutput(...)`. |
...
| `permit_exportShared(Permission perm)` | Strip sensitive fields to `SharedPermitExport`. |
