...
description: "Decrypt with a verifiable Teecryptor signature for onchain use"
---

Use `decryptForTx` to reveal a confidential (encrypted) value onchain. It returns the plaintext together with a [Teecryptor](/deep-dive/cofhe-components/teecryptor) signature, so a contract can verify the reveal when you publish it in a transaction.
...
- `signature: 0x${string}`: the Teecryptor signature as a hex string
...
Overrides the chain used to resolve the Teecryptor URL (`thresholdNetworkUrl` in the chain config) and ACPs.

### `.onPoll(callback)` (optional)

Register a callback that fires once per poll attempt while `decryptForTx` waits for Teecryptor to return the plaintext. Useful for surfacing progress in a UI.
...
| --- | --- | --- |
| `operation` | `'decrypt' \| 'sealoutput'` | Which decryption flow is polling. For `decryptForTx` this is always `'decrypt'`. |
...
Configures how long `decryptForTx` keeps retrying when Teecryptor's submit endpoint responds with `404 Not Found` before a `requestId` is available. This typically happens on slower backends where the ciphertext isn't visible yet at submit time. Defaults to `10_000` ms.
