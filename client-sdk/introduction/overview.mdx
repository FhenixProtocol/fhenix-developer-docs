...
`@cofhe/sdk` is the TypeScript client SDK for [CoFHE](/deep-dive/cofhe-components/overview). It handles the client-side work of interacting with FHE-enabled smart contracts: encrypting inputs with ZK proofs, and decrypting ciphertext handles. It also manages the EIP-712 Access Control Permissions (ACPs) that authorize those decryptions. Decryption itself happens in [Teecryptor](/deep-dive/cofhe-components/teecryptor), inside a hardware-attested TEE.
