...
`@cofhe/sdk` is the TypeScript client SDK for [CoFHE](/deep-dive/cofhe-components/overview). It handles the client-side work of interacting with FHE-enabled smart contracts: encrypting inputs with ZK proofs, and decrypting ciphertext handles via the Threshold Network. It also manages the EIP-712 Access Control Permissions (ACPs) that authorize those decryptions.
