...
| `cofhe.logMocks` in a Hardhat config | Still accepted by the plugin's config type, so it typechecks. No plugin code reads it. Use `hre.cofhe.mocks.disableLogs()` or the `setlogops` task. |
