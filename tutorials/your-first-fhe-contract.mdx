...
Anyone can now request decryption offchain using `decryptForTx`, which returns the plaintext value and a Threshold Network signature:
...
This will allow the owner to read the encrypted counter's value using the `get_encrypted_counter_value` function and decrypt it privately offchain using `decryptForView`:
