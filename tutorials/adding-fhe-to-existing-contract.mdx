...
description: "Step-by-step guide for integrating FHE capabilities into contracts you've already built"
---
...
The first thing that we need to do is import `FHE.sol` from the `cofhe-contracts` package. It gives smart contracts the FHE operations and encrypted types. We import the two encrypted types this contract stores (`euint8` and `euint64`) and the input type it receives (`externalEuint8`).

```solidity
pragma solidity ^0.8.25;

import {FHE, euint8, euint64, externalEuint8} from "@fhenixprotocol/cofhe-contracts/FHE.sol";
...
function createProposal(
    ...
    FHE.allowThis(zero);
    for (uint i = 0; i < _options.length; i++) {
        proposal.options.push(Option({name: _options[i], votes: zero}));
    }
...
The `FHE.allowThis` call matters: a trivially encrypted value is usable only inside the transaction that created it, and `vote` runs in a later transaction. Step 8 covers access in detail.

This works, but it may make sense to prepare the trivially encrypted values once, in the constructor, rather than in each transaction. That saves gas and FHE operations. Let's see how that would look:
...
<Note>
It is important to _never_ use an encrypted variable as part of an if/else branch, since the encrypted variable is always truthy. Instead, use `FHE.select` to replace the value with 0.
</Note>
...
<Warning>
It is critical to ensure that `FHE.allowThis` is used on encrypted variables that need to be used later in the contract's lifecycle. Contracts must have access to variables to perform FHE operations on them.
</Warning>
...
`FHE.allowPublic` marks each vote count as eligible for public decryption. Anyone can now request the plaintext values offchain.

### Step 10: Reveal the results with `FHE.publishDecryptResult`

After `finalizeVote` is called, the vote counts need to be decrypted offchain and published onchain with proof. The client needs the encrypted vote handles to request decryption, and `getProposal` returns option names and plaintext counts only, so we add a getter for the handles:

```solidity
function getEncryptedVotes(
    uint256 _proposalId
) external view returns (euint64[] memory votes) {
    Proposal storage proposal = proposals[_proposalId];
    if (!proposal.exists) revert ProposalNotFound();

    votes = new euint64[](proposal.options.length);
    for (uint8 i = 0; i < proposal.options.length; i++) {
        votes[i] = proposal.options[i].votes;
    }
}
```

We then add a `revealResults` function that accepts the decrypted values and the Teecryptor signatures over them:
...
The client-side flow looks like this. It assumes a connected `client` (see [client setup](/client-sdk/guides/client-setup)), viem `publicClient` and `walletClient` instances, the contract ABI as `votingAbi`, and its address as `votingAddress`:
...
// 2. Read the encrypted tallies and decrypt each one offchain
const encryptedVotes = await publicClient.readContract({
  address: votingAddress,
  abi: votingAbi,
...
const decryptedVotes: bigint[] = [];
const signatures: `0x${string}`[] = [];

for (const ctHash of encryptedVotes) {
...
// 3. Publish all results onchain
await walletClient.writeContract({
  address: votingAddress,
  abi: votingAbi,
  functionName: 'revealResults',
  args: [proposalId, decryptedVotes, signatures],
});
```
...
4. Adding a `revealResults` function that publishes decrypted values onchain with `FHE.publishDecryptResult`
5. Updating the getter function to handle decryption of results safely

The resulting contract provides the same functionality as the original, but with the added privacy benefit that individual votes are not visible onchain until the final tally is decrypted. This demonstrates how CoFHE can be used to add privacy to existing contracts with minimal changes to the core logic.
