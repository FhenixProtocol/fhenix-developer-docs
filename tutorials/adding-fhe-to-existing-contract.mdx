...
<Note>
Encrypting inputs requires the use of [**cofhejs**](/cofhejs/introduction/overview).

Read more about [**encrypted inputs**](/cofhejs/guides/encryption).
</Note>
...
### Step 9: Finalize the voting with `FHE.decrypt`

Because the votes are encrypted for the lifetime of the proposal, after the proposal has ended, we need to decrypt the results and reveal the winner. Let's start by adding a `finalizeVote` function:
...
### Step 10: Checking the results with `FHE.getDecryptResultSafe`
...
- Learn about [Migrating to CoFHE](/tutorials/migrating-to-cofhe) for more advanced scenarios
