...
<Note>
The key difference: `FHE.decrypt` triggered decryption without any proof. The new flow requires a Threshold Network signature, ensuring the plaintext is cryptographically verified before being used onchain.
</Note>
...
**Key differences:**
- `FHE.decrypt(burned)` to `FHE.allowPublic(burned)`: no onchain decryption is triggered, just a permission grant
...
- `FHE.getDecryptResultSafe` to `FHE.publishDecryptResult`: the contract verifies the Threshold Network signature instead of polling for a result
