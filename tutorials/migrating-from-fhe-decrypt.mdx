...
<Note>
The key difference: `FHE.decrypt` triggered decryption without any proof. The new flow returns a signature from [Teecryptor](/deep-dive/cofhe-components/teecryptor), the component that decrypts CoFHE ciphertexts inside a hardware-attested TEE. Your contract verifies that signature before it uses the plaintext.
</Note>
