...
|-----------|-------------|
| **Your app** | The application that interacts with the user and the contracts |
| **Client SDK** (`@cofhe/sdk`) | TypeScript library that encrypts inputs locally and submits them for verification |
| **Your contract** | Consumes the handles and the proof, and forwards them to the TaskManager |
...
| **[ZK Verifier](/deep-dive/cofhe-components/zk-verifier)** | Attested offchain component that verifies the proofs of every encrypted-input batch |
| **CT Server** | Stores the verified ciphertext bytes for the coprocessor |
| **[Compute pipeline](/deep-dive/cofhe-components/compute-pipeline)** | Picks up each `InputVerified` event and posts the commitment |
| **[CommitmentRegistry](/deep-dive/cofhe-components/commitment-registry)** | Records a commitment for every verified input |

## Flow diagram

{/* Diagram source: encryption-request-flow.mmd, next to this page. Edit it first, then re-run the archify build. */}
