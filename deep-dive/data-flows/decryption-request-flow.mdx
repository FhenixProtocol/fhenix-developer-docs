...
The request goes to Teecryptor with the handle, the host chain id, and the ACP. For publicly decryptable handles, `decryptForTx` can use `.withoutPermit()` instead.
</Step>
...
sequenceDiagram
    ...
    alt decryptForTx
        ...
        Teecryptor-->>SDK: result sealed to the ACP's sealing key
