...
| --- | --- |
| CommitmentRegistry address | The contract, its interface, and the version tag are settled, and the coprocessor already posts commitments on the networks that run today. The chain and address the public network will use are not settled yet, so there is nothing stable to publish here. |
