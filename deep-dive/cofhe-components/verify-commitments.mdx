...
| Value | Status |
| --- | --- |
| CommitmentRegistry address | The contract, its interface, and the version tag are settled, and the coprocessor already posts commitments on the networks running today. The chain and address for the public network are being finalised, and they are published here as soon as they are confirmed. |
...
The second is the version. More than one version can be Active at once, and a handle committed before a cutover resolves only under the version it was posted with. Read the status of each version you know of, and retry the lookup under the earlier one before you conclude anything.
...
[Teecryptor](/deep-dive/cofhe-components/teecryptor) makes the same comparison inside its enclave on every decryption request. It hashes the ciphertext bytes it is about to decrypt and refuses any that do not match the commitment anchored onchain. Your audit and its gate read the same record, so a commitment you can check yourself is the same one that gates every decryption.
