...
<Warning>
On the networks running today the gate is configured warn-only: a mismatch is logged and the decryption still proceeds. Enforcement is the intended setting, and it turns on once handles committed under the earlier version have aged out. So treat the onchain commitment as the record you audit, and do not assume a mismatch is currently refused.
</Warning>
