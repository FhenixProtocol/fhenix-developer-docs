...
<Tip>
Colour marks the zone. The keygen ceremony and the consumer enclave run inside CoFHE. The partners are independent custodians, so they are not coloured as CoFHE's. Hover any component or message in the [explorable version](/deep-dive/cofhe-components/key-management-explorable).
</Tip>
