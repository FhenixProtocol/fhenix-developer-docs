...
Contracts read results back with `FHE.getDecryptResult(handle)`, which reverts if the result is not yet published, or `FHE.getDecryptResultSafe(handle)`, which returns a ready flag instead. See the [Decryption Request Flow](/deep-dive/data-flows/decryption-request-flow) for the full path.
