---
title: ZK Verifier
description: "Offchain component that verifies user inputs using Zero-Knowledge Proofs of Knowledge (ZKPoK) to ensure encrypted data is safe to use in smart contracts"
---
...
| Aspect | Description |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Type** | Offchain component running inside a hardware-attested TEE. |
...
{/* Diagram source: zk-verifier.mmd, beside this page. Edit it first, then re-run the archify build. */}
<Frame>
<img src="/images/zk-verifier.svg" alt="Sequence diagram of the zk verifier path" style={{ width: "100%" }} />
</Frame>

<Tip>
Color marks the zone. Your app and the Client SDK run client side, your contract and the TaskManager on the host chain. The ZK Verifier and compute pipeline run offchain inside CoFHE, and the CommitmentRegistry sits on the registry chain. Hover any component or message in the [explorable version](/deep-dive/cofhe-components/zk-verifier-explorable).
</Tip>
...
6. The TaskManager verifies the batch signature and emits `InputVerified` per input. The [compute pipeline](/deep-dive/cofhe-components/compute-pipeline) picks those events up and posts a commitment for each input to the [CommitmentRegistry](/deep-dive/cofhe-components/commitment-registry), so the input becomes decryptable later.
