...
sequenceDiagram
    ...
    App->>SDK: encrypt the inputs, generate a zkPoK for each
    SDK->>ZK: verifyBatch (ciphertexts + proofs)
    ZK->>ZK: verify every proof in the attested TEE
    ZK->>ZK: store the ciphertext bytes in CT Server
    ZK->>ZK: archive the inputs and proofs for public audit
    ZK->>ZK: sign one digest over the whole batch
    ZK-->>SDK: handles + batch signature
    SDK-->>App: [handles..., proof]
    App->>C: contract call with handles + proof
    C->>TM: batchVerifyInputs
    TM->>TM: ecrecover against verifierSigner
