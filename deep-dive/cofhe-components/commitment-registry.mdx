...
A commitment is the `keccak256` hash of a ciphertext's canonical stored bytes. The [compute pipeline](/deep-dive/cofhe-components/compute-pipeline) produces one for every result it computes, and anchors one for every encrypted input the [ZK Verifier](/deep-dive/cofhe-components/zk-verifier) approves. It batches them and writes them to this registry.

## Who writes and who reads

One component writes commitments, and one component reads them. The write happens when a ciphertext is produced; the read happens later, whenever someone asks to decrypt it.

{/* Diagram source: commitment-registry.mmd, beside this page. Edit it first, then re-run the archify build. */}
<Frame>
<img src="/images/commitment-registry.svg" alt="Sequence diagram of the commitment write and read" style={{ width: "100%" }} />
</Frame>

<Tip>
The compute pipeline and Teecryptor run offchain inside CoFHE; the registry itself is onchain. Hover any component or message in the [explorable version](/deep-dive/cofhe-components/commitment-registry-explorable).
</Tip>
...
`version` is an opaque `bytes32` tag chosen by the coprocessor when FHE parameters change. The current tag is the number 2 encoded as `bytes32`. Every version moves through a small state machine:

<Frame>
<img src="/images/commitment-version.svg" alt="State machine for a commitment version: Unset to Active to Deprecated, with Revoked as the terminal state" style={{ width: "100%" }} />
</Frame>

<Tip>
Only an Active version accepts new commitments. Revoked is terminal.
</Tip>
...
- Offchain: the compute pipeline's blockchain poster batches the commitments and posts them to this registry.
- The pipeline sets the commitment version, and bumps it when the FHE parameters change.
