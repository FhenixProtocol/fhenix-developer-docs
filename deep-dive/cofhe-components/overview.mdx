...
flowchart TB
    subgraph Yours["Your code (outside CoFHE)"]
        FHEC["Your contract, uses FHE.sol"]
    end
    ...
    subgraph CoFHE["CoFHE (offchain)"]
        ...
        subgraph ENG["FHE Engine"]
            direction TB
            L["subscribe to task events"] --> X["process FHE computation"] --> P["post commitments"]
        end
    end
    ...
    ENG <-- "ciphertexts" --> CTS
    TM -- "task events" --> ENG
    ENG -- "post commitments" --> CR
    PRT -- "key shares (attested)" --> TEE
    PRT -- "signing key (attested)" --> ZK

    linkStyle 2,10 stroke:#7FDB8F
    linkStyle 3,4,7,8,9 stroke:#0AD9DC
