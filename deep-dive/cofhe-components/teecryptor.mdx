...
sequenceDiagram
    ...
    and Commitment lookup
        ...
        CR-->>TEE: expected hash
    end
    TEE->>CTS: GetStoredCt(handle)
    CTS-->>TEE: ciphertext bytes
    TEE->>TEE: check the bytes hash to the commitment
    TEE->>TEE: decrypt in the enclave, sign or seal the result
    TEE-->>SDK: plaintext + signature, or sealed output
```
