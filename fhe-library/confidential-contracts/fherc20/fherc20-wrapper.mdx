...
The user (or anyone) requests decryption of the burned amount offchain via `decryptForTx`, receiving the plaintext and a decryption proof.
</Step>

<Step title="Claim Tokens">
The user submits the plaintext and proof onchain via `claimUnshielded`. The contract verifies the proof and transfers the underlying ERC20 tokens (multiplied by the rate).
</Step>
</Steps>
...
Unshielding is a three-step process shared by both wrapper types: burn onchain, decrypt offchain, then claim with proof.
...
Retrieve the claim's `ctHash` using `getUserClaims`, then request decryption via `decryptForTx`. Since `FHE.allowPublic` was called, no [Access Control Permission](/client-sdk/guides/acps) (ACP) is needed:
...
// Request decryption offchain (no ACP needed)
...
Claiming requires completing the offchain decryption step first:
