...
Anyone can call `decryptForTx` offchain to obtain the plaintext values and Threshold Network signatures for the winning bid and bidder.
</Step>
...
The `revealWinner` function accepts the decrypted values and their Threshold Network signatures, then publishes them onchain:
...
Decryption happens offchain via `decryptForTx`, and results are verified onchain via `FHE.publishDecryptResult` with a Threshold Network signature.
</Card>
</CardGroup>
