...
Publishes a decrypted result onchain by verifying the Threshold Network signature. The plaintext is stored and can be read via `getDecryptResultSafe`.
...
Verifies a Threshold Network signature **without** storing the plaintext onchain. Returns `bool`.
