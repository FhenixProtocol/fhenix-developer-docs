...
Each of these takes the input handle and the `bytes` proof that authenticates it. One signature covers every encrypted input in a call, so if a function receives more than one, convert them together with the plural form (`asEuint8s`, `asEuint32s`, and so on) rather than calling the single form per value.

#### asEbool (from externalEbool)

<ParamField body="hash" type="externalEbool" required>
Encrypted input handle containing boolean data
</ParamField>

<ParamField body="proof" type="bytes" required>
Signature authenticating the input batch
</ParamField>
...
Converts an encrypted input into an encrypted boolean.

```solidity
ebool encrypted = FHE.asEbool(encryptedInput, inputProof);
```

#### asEuint8 (from externalEuint8)

<ParamField body="hash" type="externalEuint8" required>
Encrypted input handle containing 8-bit integer data
</ParamField>
...
Converts an encrypted input into an encrypted 8-bit unsigned integer.

```solidity
euint8 encrypted = FHE.asEuint8(encryptedInput, inputProof);
```

#### asEuint16 (from externalEuint16)

<ParamField body="hash" type="externalEuint16" required>
Encrypted input handle containing 16-bit integer data
</ParamField>
...
Converts an encrypted input into an encrypted 16-bit unsigned integer.

```solidity
euint16 encrypted = FHE.asEuint16(encryptedInput, inputProof);
```

#### asEuint32 (from externalEuint32)

<ParamField body="hash" type="externalEuint32" required>
...
Converts an encrypted input into an encrypted 32-bit unsigned integer.

```solidity
euint32 encrypted = FHE.asEuint32(encryptedInput, inputProof);
```

#### asEuint64 (from externalEuint64)

<ParamField body="hash" type="externalEuint64" required>
Encrypted input handle containing 64-bit integer data
</ParamField>
...
Converts an encrypted input into an encrypted 64-bit unsigned integer.

```solidity
euint64 encrypted = FHE.asEuint64(encryptedInput, inputProof);
```

#### asEuint128 (from externalEuint128)

<ParamField body="hash" type="externalEuint128" required>
...
Converts an encrypted input into an encrypted 128-bit unsigned integer.

```solidity
euint128 encrypted = FHE.asEuint128(encryptedInput, inputProof);
```

#### asEaddress (from externalEaddress)

<ParamField body="hash" type="externalEaddress" required>
Encrypted input handle containing address data
</ParamField>
...
Converts an encrypted input into an encrypted address.

```solidity
eaddress encrypted = FHE.asEaddress(encryptedInput, inputProof);
```
...
Publishes multiple decrypted results onchain in a single call. Each element is verified independently.
...
Grants public permission to access the encrypted value. Once called, anyone can request decryption of this value offchain via `decryptForTx` without needing an ACP.
...
// Now anyone can call decryptForTx offchain without an ACP
```
...
inputs[0] = encryptedInputA;   // Provided by client-side encryption
inputs[1] = encryptedInputB;   // Provided by client-side encryption
