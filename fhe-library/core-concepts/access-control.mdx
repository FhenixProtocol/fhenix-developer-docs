...
The ACL governs **operation creation** — computing on a handle, and requesting its decryption. It does not gate the on-chain publication of a decrypt result:
...
`publishDecryptResult` (and `verifyDecryptResult`) only check that `signature` is a valid ECDSA signature from the Threshold Network over that exact `(ctHash, result, chainId, encType)` tuple. Publication is deliberately permissionless so clients and relayers can settle results on-chain.
...
<Warning>
Treat `FHE.allowPublic()` as the real decision point. Once a handle is marked public, anyone can request its decryption and publish the plaintext on-chain, permanently.
</Warning>
