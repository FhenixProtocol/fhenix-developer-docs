...
1. A client requests decryption offchain and receives the plaintext along with a signature from Teecryptor, the component that decrypts CoFHE ciphertexts.
2. The plaintext and signature are submitted onchain, where the contract publishes or verifies the result.

<Tip>
Teecryptor decrypts inside a hardware-attested Trusted Execution Environment (TEE), and signs each result with a key that never leaves the enclave. See the [Teecryptor](/deep-dive/cofhe-components/teecryptor) guide for the full request path. A Threshold Network that decrypts through multi-party computation is planned to replace it, and is described in [Future Plans](/deep-dive/research/future-plans).
</Tip>
